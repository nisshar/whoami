...
\
    try \{\
      ...
      console.log('\uc0\u9989  Visitor captured:', visitorData);\
    \} catch (error) \{\
      console.error('\uc0\u10060  Error saving visitor:', error);\
    \}\
  \};\
\
...
\
      ...
      \});\
      \
      console.log('\uc0\u55357 \u56522  Admin data loaded:', \{ messages: msgList.length, visitors: visList.length \});\
    \} catch (error) \{\
      console.error('\uc0\u10060  Error loading admin data:', error);\
    \}\
    setLoading(false);\
  \};\
\
...
\
    try \{\
      ...
      \
      console.log('\uc0\u9989  Message saved:', newMessage);\
      setSubmitted(true);\
      setMessage('');\
      setTimeout(() => setSubmitted(false), 3000);\
    \} catch (error) \{\
      console.error('\uc0\u10060  Error saving message:', error);\
...
\
  const handleAdminLogin = () => \{\
    if (password === 'admin123') \{\
      ...
      setPassword('');\
      console.log('\uc0\u9989  Admin logged in');\
    \} else \{\
      alert('\uc0\u10060  Incorrect password!');\
      setPassword('');\
    \}\
  \};\
\
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">\
        <div className="max-w-7xl mx-auto p-4 md:p-8">\
          <div className="flex justify-between items-center mb-8">\
            <div>\
              <h1 className="text-3xl font-bold text-white mb-2">\uc0\u55356 \u57263  Admin Dashboard</h1>\
              <p className="text-purple-300">Monitor visitors and messages</p>\
            </div>\
            <button\
              onClick=\{() => \{\
                setIsAdmin(false);\
                console.log('\uc0\u55357 \u56595  Logged out from admin');\
              \}\}\
                      ...
                      <div key=\{msg.id\} className="bg-white/5 rounded-lg p-4 border border-white/10 hover:bg-white/10 transition">\
                        ...
                        <div className="flex flex-wrap gap-3 text-sm text-purple-300">\
                          <span className="flex items-center gap-1">\
                            \uc0\u55357 \u56517  \{new Date(msg.timestamp).toLocaleString()\}\
                          </span>\
                          <span>\'95</span>\
                          <span>\uc0\u55357 \u56561  \{msg.device || getDeviceType(msg.userAgent)\}</span>\
                          <span>\'95</span>\
                          <span>\uc0\u55356 \u57104  \{msg.browser || getBrowser(msg.userAgent)\}</span>\
                        </div>\
                      </div>\
                    ))\
                  )\}\
...
\
          \{submitted ? (\
            <div className="text-center py-12">\
              ...
              <h2 className="text-2xl font-bold text-white mb-2">\uc0\u9989  Message Sent!</h2>\
            ...
            <div className="space-y-6">\
              <div className="flex flex-wrap gap-2 mb-4">\
                <p className="w-full text-purple-200 text-sm mb-2">\uc0\u55357 \u56481  Quick starters:</p>\
...
\
        <p className="text-center text-purple-300 mt-6 text-sm">\
          \uc0\u55357 \u56592  Powered by anonymous messaging \'95 Your privacy matters\
        </p>\
      </div>\
    </div>\
  );\
